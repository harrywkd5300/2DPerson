...
CLeeSoonsin::CLeeSoonsin()
	:m_fScaleX(1.f), m_fMoveTime(0.f), m_fAttackTime(0.f), m_bMove(false), m_bStopMove(false), fDist(0.f)
{
	for (int i = 0; i < 10; ++i)
		m_Inven[i] = NULL;
...
HRESULT	CLeeSoonsin::Initialize(void)
{
	m_tFrame = FRAME(0.f, 8.f, 8.f);


	m_tUnit.vPos = D3DXVECTOR3(300.f, 300.f, 0.f);
	m_tUnit.vSize = D3DXVECTOR3(66.f, 45.f, 0.f);
	lstrcpy(m_tUnit.wstrObjKey, L"LeeSunsin");
	...
	m_CollRect.left = int(m_tUnit.vPos.x   + m_vScroll.x - 20);
	m_CollRect.top = int(m_tUnit.vPos.y    + m_vScroll.y - 20);
	m_CollRect.right = int(m_tUnit.vPos.x  + m_vScroll.x + 20);
	m_CollRect.bottom = int(m_tUnit.vPos.y + m_vScroll.y + 20);
...
void CLeeSoonsin::Release(void)
{
	safeDelete(AStar);
}

void CLeeSoonsin::MyFrameCheck()
{
...
void CLeeSoonsin::StateChange()
{
	if (m_eCurState != m_ePreState)
	{
		...
		case Attack_LD:
			...
			break;
		case Skill_up:
			m_tFrame = FRAME(0.f, 8.f, 11.f);
			lstrcpy(m_tUnit.wstrStateKey, L"Skill4");
			m_fScaleX = 1.f;
			break;
		case Skill_down:
			m_tFrame = FRAME(0.f, 8.f, 11.f);
			lstrcpy(m_tUnit.wstrStateKey, L"Skill5");
			m_fScaleX = 1.f;
			break;
		case Skill_right:
			m_tFrame = FRAME(0.f, 8.f, 11.f);
			lstrcpy(m_tUnit.wstrStateKey, L"Skill2");
			m_fScaleX = -1.f;
			break;
		case Skill_left:
			m_tFrame = FRAME(0.f, 8.f, 11.f);
		...
		case Skill_RU:
			m_tFrame = FRAME(0.f, 8.f, 11.f);
			lstrcpy(m_tUnit.wstrStateKey, L"Skill3");
			m_fScaleX = -1.f;
			break;
		case Skill_RD:
			m_tFrame = FRAME(0.f, 8.f, 11.f);
			lstrcpy(m_tUnit.wstrStateKey, L"Skill1");
			m_fScaleX = -1.f;
			break;
		case Skill_LU:
			m_tFrame = FRAME(0.f, 8.f, 11.f);
...
void CLeeSoonsin::AStarMove()
{
	...
	if (m_bClickUnit)
	{
		if (m_bBattle)
		{
			...
			{
				if (m_iWay == WAY_UP)
					m_eCurState = Skill_up;
				else if (m_iWay == WAY_DOWN)
					m_eCurState = Skill_down;
				else if (m_iWay == WAY_RIGTH)
					m_eCurState = Skill_right;
				else if (m_iWay == WAY_LEFT)
					m_eCurState = Skill_left;
				else if (m_iWay == WAY_RIGTHUP)
					m_eCurState = Skill_RU;
				else if (m_iWay == WAY_RIGTHDOWN)
					m_eCurState = Skill_RD;
				else if (m_iWay == WAY_LEFTUP)
					m_eCurState = Skill_LU;
				else if (m_iWay == WAY_LEFTDOWN)
					m_eCurState = Skill_LD;
				return;
			}
			if (m_bAttackTime)
			{
...
bool CLeeSoonsin::Picking(const D3DXVECTOR3& vPos)
{
	//타일의 위치벡터
...
void CLeeSoonsin::BuyItem(TCHAR strkey[MIDDLE_STR])
{
	for (int i = 0; i<10; ++i)
	{
		if (m_Inven[i] == NULL)
