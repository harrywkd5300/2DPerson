...
CFireBird::CFireBird()
	:m_fScaleX(1.f), m_fMoveTime(0.f), m_fAttackTime(0.f), m_bStopMove(false), fDist(0.f)
{
...
void CFireBird::Render(void)
{
	...
	//D3DXMatrixScaling(&matScale, 40.f, 40.f, 0.f);

	//m_tUnit.matWorld = matScale * matTrans;

	//CDevice::GetInstance()->GetSprite()->SetTransform(&m_tUnit.matWorld);

	//float fX2 = CENTER_POS(pRectTex->ImageInfo.Width);
	//float fY2 = CENTER_POS(pRectTex->ImageInfo.Height);
...
void CFireBird::Release(void)
{
	safeDelete(AStar);
}

void CFireBird::MyFrameCheck()
{
...
void CFireBird::KeyInput()
{
	...
	//vMousePos -= D3DXVECTOR3(70.f, 0.f, 0.f);
	...
	else if (KEYPRESS(dwKey, A))
	{
		if (SelectLeeSoonSin == FALSE)
		{
			...
			D3DXVECTOR3 vDirPos = D3DXVECTOR3(0.f, 0.f, 0.f);

			if (m_iWay == WAY_UP)
				vDirPos = D3DXVECTOR3(0.f, -30.f, 0.f);
			else if (m_iWay == WAY_DOWN)
				vDirPos = D3DXVECTOR3(0.f, 30.f, 0.f);
			else if (m_iWay == WAY_RIGTH)
				vDirPos = D3DXVECTOR3(30.f, 0.f, 0.f);
			else if (m_iWay == WAY_LEFT)
				vDirPos = D3DXVECTOR3(-30.f, 0.f, 0.f);
			else if (m_iWay == WAY_RIGTHUP)
				vDirPos = D3DXVECTOR3(30.f, -50.f, 0.f);
			else if (m_iWay == WAY_RIGTHDOWN)
				vDirPos = D3DXVECTOR3(20.f, 20.f, 0.f);
			...
			CObjMgr::GetInstance()->AddGameObject(
				CFactory<CBomb>::CreateGameObject(m_tUnit.vPos.x + vDirPos.x,  m_tUnit.vPos.y + vDirPos.y, 0.f),
				OBJ_TYPE_EFFECT);
		}
	}
	else
...
bool CFireBird::Picking(const D3DXVECTOR3& vPos)
{
	//타일의 위치벡터
